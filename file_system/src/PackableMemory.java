...
class PackableMemory
{
   ...
   String unpackName(int loc){
	   int counter = 4;
	   for(int i = 0; i < 4; ++i)
		   if(mem[loc + i] == (byte) -1)
			   counter--;
	   return new String(mem, loc, counter);
   }
   ...
   public void setBytes(byte[] bytes, int i) {
	   for(byte b : bytes){
		   mem[i++] = b;
	   }
   }
   
   public byte getMemByteAt(int index){
	   return mem[index];
   }
   
   public void setMemByteAt(int index, byte val){
	   mem[index] = val;
   }
